...
function makeRoom(roomString) {
  const splitData = roomString.split('-');
  const name = splitData.slice(0, splitData.length-1).join('');
  const ogName = splitData.slice(0, splitData.length-1).join(' ');
  const roomNumber = splitData[splitData.length-1].split('[')[0];
  const checkSum = roomString.match(/\[([^\]]+)\]/)[1];
...
function getLetterCounts(roomName) {
  return roomName.split('').reduce((acc, val) => {
    if (!acc[val]) {
      acc[val] = 1;
    } else {
      acc[val]++;
...
function calculateChecksum(room) {
  const letterCounts = getLetterCounts(room.name);
  const pairs = Object.keys(letterCounts).map((letter) => ([letterCounts[letter], letter]));
  ...
  return sorted.map((pair) => pair[1]).join('').slice(0,5);
}

function validateRoom(room) {
  return room.calculatedChecksum === room.checkSum;
}

// lower case letters go from 97 -> 122
function rotateLetter(letter, num) {
  if (letter === ' ') {
    return letter;
  }
...
function decryptRoom(room) {
  ...
  room.ogName.split('').forEach((char) => {
    decryptedRoom.push(rotateLetter(char, Number(room.roomNumber)));
  });
...
const result = realInput
  ...
  .map((room) => {
    // such mutation, wow
    room.calculatedChecksum = calculateChecksum(room);
    return room
  })
  .filter(validateRoom)
  .map((validRoom) => {
    validRoom.decryptedRoom = decryptRoom(validRoom);
    return validRoom;
  })

fs.writeFileSync('decryptedRoomList.json', JSON.stringify(result, null, '  '));